...
\end{equation}
```
With $g$ being the absolute deviation from 50% responding to an item correctly and 50% responding incorrectly (e.g., a "*p*-value" of .5). $z_g$ is the standard normal deviate associated with $g$. This transformation of the common *p*-value was recommended by @kulas2017approximate in order to scale the CTT index along a (closer to) interval-level metric more directly analogous to the IRT *b*-parameter. The current investigation retained the @kulas2017approximate formulas as "starting points" -- because our intent was to generate ICCs comparable to IRT-derived ICCs, we also amended the @kulas2017approximate formulas to place $z_g$ and $\hat{a_i}$ on metrics aligned with the IRT *b* and *a* parameters. 
