...
abstract: |
  Item characteristic curves (ICC's) provide a visualization of important attributes of assessment items - most commonly *difficulty* and *discrimination*. Assessment specialists who examine ICC's usually do so from within the psychometric framework of either Item Response Theory (IRT) or Rasch modeling. We propose an extension of this tradition of item characteristic visualization within the Classical Test Theory (CTT) framework. We first simulate binary (e.g., true *test*) data with controlled item difficulty characteristics to generate linking coefficients between the IRT and CTT difficulty indices. The results of these simulations provided some degree of confidence regarding functional linking invariance. Next, we simulated datasets of varying item characteristic specification and generated ICCs derived from both IRT and CTT frameworks. Differential item functioning (DIF) was estimated by calculating the geometric area between the IRT- and CTT-derived ogives. The DIF was .... An `R` package, `ctticc`, performs the ICC estimations presented in the current paper and provides assessment specialists with visual representations of CTT-derived item characteristics.
...
@lord1980applications described a function ^[Lord] that approximates the nonlinear relationship between the IRT *a*-parameter and the CTT discrimination index:

$$a_i\cong \frac{r_i}{\sqrt{1-r_i^2}}$$
^[Lord]: @lord1980applications's CTT discrimination index is actually the item-test biserial correlation as opposed to the more contemporarily more popular *corrected* item-total *point-biserial* correlation. 
