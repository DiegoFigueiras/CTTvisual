...
A regression predicting b from $z_g$ showed a $R^2$=`r reg1sum$r.squared`. Regarding possible differences across simulation conditions, a moderated regression yeilded a significant interaction effect ($F_{(1, 4,943,349)}$=`r r2change$F[2]`, *p* < .001), but this was due to our exceedingly large sample size ($\Delta{R^2}$=`r reg3sum$r.squared-reg1sum$r.squared`). $\leftarrow$ [WE NEED TO REPORT THE STEP 1 AND STEP 2 RESULTS AS WELL]
...
The purpose of Study 2 was to evaluate the comparability of IRT- and CTT-derived ICCs. We generated ICCs from one simulated and six real-world test datasets. The real-world datasets represent responses from the Test of English as a Foreign Language (TOEFL) iBT$\leftarrow$[WHAZZAT?]. The two TOEFL ITP datasets come from two test forms and include item-level scores. $\leftarrow$[REWORD - explain "form"] This test has subscales that assess different skills: reading (k=39), listening (k=40), and speaking (k=35). Each dataset includes responses from 10,000 examinees, and the examinee populations for the two forms do not overlap. [WE NEED TO EXPLAIN THE WINGEN DATA - ALSO TOEFL A BIT MORE]
