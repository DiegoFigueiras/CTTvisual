...
library(ggplot2)
ggplot(data = coeficients, aes(x = intercept)) + geom_histogram(bins = 500) + facet_grid(simulation~.)+xlim(-.25,.25)
...
ggplot(data = coeficients, aes(x = slope)) + geom_histogram(bins = 500) + facet_grid(simulation~.)+xlim(-2,-1)
...
ggplot(data = auc_together, aes(x = auc)) + geom_histogram(bins=30, fill="cyan", color="black") + facet_grid(condition~.)+geom_density(color="red")

auc_together$condition<-as.factor(auc_together$condition)
auc_together<-as.table(auc_together)
