...
Figure \@ref(fig:stackedplot) presents the distribution of five million regression slopes and intercepts estimated across our simulations. A regression predicting *b* from $z_g$ returned a $R^2$ value of `r reg1sum$r.squared`. Regarding possible differences across simulation conditions, a moderated hierarchical regression did yield a significant interaction effect ($F_{(1, 4,943,349)}$ = `r round(r2change$F[2], 5)`, *p* < .001), but this was due to our large sample size ($\Delta{R^2}$ = `r round(reg3sum$r.squared-reg1sum$r.squared,5)`). 
...
3. $b_{pseudo}$ = $1.54 * z_g$  ## Check with actual package specification
