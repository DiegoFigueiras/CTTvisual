...
The noted geometric areas between curves for all investigated items was, on average, low. This represents some promise regarding plotted ICC's using IRT and CTT parameters. Our hypothesis was that the area between Curves of these different ICCs would be small. Area between curves for 100 items was 0.35 on average. This result indicates that curves plotted with either IRT or CTT parameters show little difference. The nature of both models is overlapping when it comes to plotting visual representations such as ICC's. Practitioners and researchers that don't use IRT or Rasch models and instead opt to follow a CTT philosophy would benefit from having ICC's that use CTT statistics. The current specifications are available to apply via a small `R` package called `ctticc`. Because the ICCs are derived from the same individuals, the current application does not require the same parameter scaling that is typically required in DIF investigations where the ICCs reflect responses from different groups that likely have different underlying distributions of ability.  
...
IRT analyses also require large samples of data. These CTT-derived ICC estimates may be useful to individuals who wish to ultimately apply IRT, but are limited in their data sample size, as is the case for example in classroom settings. 

IRT models can converge with wildly large *b*-parameter estimates with extremely difficult or easy items @baker2004item. This is possibly the reason previous investigations have noted an advantage to the CTT-derived difficulty estimates with regard to index invariance [e.g., @fan1998item; @lawson1991one]. The IRT estimate is bound by positive and negative infinity - the CTT estimate is finite and does not suffer this possibility.
