...
[^4]: And in fact, as is presented below, the relationship between the IRT and CTT discrimination indices is non-linear. The Pearson's product moment correlation is not the most appropriate index to capture the extent of the magnitude of this relationship.
...
We retained the @kulas2017approximate formulas as "starting points". Because our intent was to generate ICCs that could be directly contrasted with IRT-derived ICCs, the following procedures also amended the @kulas2017approximate formulas to place $z_g$ and $\hat{a_i}$ on scales more closely aligned with the IRT *b* and *a* parameters.  
...
A regression predicting b from $z_g$ returned a $R^2$ value of `r reg1sum$r.squared`. Regarding possible differences across simulation conditions, a moderated regression resulted in a significant interaction effect ($F_{(1, 4,943,349)}$ = `r r2change$F[2]`, *p* < .001), but this was due to our large sample size ($\Delta{R^2}$ = `r reg3sum$r.squared-reg1sum$r.squared`).
...
The purpose of Study 2 was to evaluate the comparability of IRT- and CTT-derived ICCs. We generated ICCs from one simulated and six real-world test datasets. The real-world datasets represent responses from the Test of English as a Foreign Language institutional testing program (TOEFL ITP). There were two different test forms administered of this assessment. The TOEFL ITP has subscales of: reading (k=39), listening (k=40), and speaking (k=35). Two datasets representing responses to items defining the three subscales both include responses from 10,000 examinees, and the examinee samples for the two forms do not overlap. 

The simulated data were generated using Wingen [@han2007wingen3]. The researchers requested a dataset with 100 binary response items and 10,000 "respondents". Because we wanted a range of universally positive item discrimination values to model, we generated responses derived from a normal and centrally located distribution of "ability" and items with a mean *a*-parameter value of 2 (sd = 0.8) and a mean *b*-parameter value of 0 (sd = 0.5). 

Differential item functioning (DIF) was estimated via directly calculating the area between ICCs. For the current study, DIF was calculated along a two-dimensional plotting space defined by an x-axis ranging from -6 to 6 and a y-axis ranging from 0 to 1, creating a maximum possible 2-dimensional area of 13. $\leftarrow$[should probably also do a standard DIF estimate - take from @millsap2012statistical]. 
...
The average DIF across all 7 datasets was `r mean(auc_together$auc)` (sd=`r sd(auc_together$auc)`). There was no difference in average DIF across the 7 datasets ($F_{(6, 321)}$ =`r auc_anova[1,4]`, *p* \> .05). The range of DIF values had a smallest value of `` and a largest values of ``. Only 17 (5.2%) of the 328 total investigated items exhibited a DIF above 0.3. Figure \@ref(fig:histrogram) presents histograms of the individual DIF magnitudes organized by focal test. The simulated test data returned an average DIF estimate of `r mean(data0$diff)` whereas the TOEFL tests returned an average estimate of `r mean(df_plot$diff)` across `r nrow(df_plot)` investigated items. 
